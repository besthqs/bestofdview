  ...
  for (const stamp of stampAnnot) {
    if (stamp.sealObj && Object.keys(stamp.sealObj).length > 0) {
      if (stamp.sealObj.type === "ofd") {
        ...
        for (const stampObj of stampObjs) {
        ...
        const img =
          ...
          btoa(String.fromCharCode.apply(null, stamp.sealObj.ofdArray));
        let stampArray = [];
  ...
  for (const anno of annotations) {
    ...
    if (zip.files[fileLoc]) {
      ...
      for (const annot of array) {
        ...
        const appearanceObj = { type, appearance, visible };
...
export const getPublicRes = async function ([
  ...
  if (publicResPath) {
    ...
    if (zip.files[publicResPath]) {
      ...
      const fontObj = await getFont(publicResObj);
      fontResObj = Object.assign(fontResObj, fontObj);
      const drawParamObj = await getDrawParam(publicResObj);
      drawParamResObj = Object.assign(drawParamResObj, drawParamObj);
      const multiMediaObj = await getMultiMediaRes(zip, publicResObj, doc);
...
export const getTemplatePage = async function ([
  ...
  const templatePages = Document["ofd:CommonData"]["ofd:TemplatePage"];
  let array = [];
  array = array.concat(templatePages);
  const tpls = {};
  for (const templatePage of array) {
    if (templatePage) {
      const pageObj = await parsePage(zip, templatePage, doc);
...
export const getPage = async function ([
  ...
  const pages = Document["ofd:Pages"]["ofd:Page"];
  let array = [];
  array = array.concat(pages);
  const res = [];
  for (const page of array) {
    if (page) {
      const pageObj = await parsePage(zip, page, doc);
...
const getCompositeObject = function (res) {
  ...
  const compositeGraphicUnitsArray = [];
...
const getSignature = async function (zip, signatures, doc) {
  ...
  if (signatures) {
    ...
    if (zip.files[signatures]) {
      const data = await getJsonFromXmlContent(zip, signatures);
      const signature = data["json"]["ofd:Signatures"]["ofd:Signature"];
      let signatureArray = [];
      signatureArray = signatureArray.concat(signature);
      for (const sign of signatureArray) {
        if (sign) {
          let signatureLoc = sign["@_BaseLoc"];
          const signatureID = sign["@_ID"];
...
const getSignatureData = async function (zip, signature, signatureID) {
  ...
  const sealObj = await parseSesSignature(zip, signedValue);
  const checkMethod =
  ...
  return new Promise((resolve, reject) => {
    zip.files[xmlName].async("string").then(
      function (content) {
        ...
        const jsonObj = parser.parse(content, ops);
        const result = { xml: content, json: jsonObj };
        resolve(result);
      },
      function error(e) {
        reject(e);
