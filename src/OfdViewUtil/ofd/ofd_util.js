...
/**
 * 2024.9.23 @auther 侯庆松 字体匹配规则
 * @param {*} font
 * @param {*} text  内容，如果是字母数字，要返回 Lekton
 * @returns
 */
export const getFontFamily = function (font, text) {
  //2024.9.23 @auther 侯庆松 字体匹配规则,如果是字母数字，返回 Lekton
  if (/^[a-zA-Z0-9/.]+$/.test(text)) return "'Lekton', sans-serif";

  if (FONT_FAMILY[font.toLowerCase()]) {
    font = FONT_FAMILY[font.toLowerCase()];
    //2024.6.8 @auther 侯庆松 直接返回
    return font;
  }
